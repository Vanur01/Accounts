  ...
  return (
    <div className="max-w-5xl mx-auto rounded-lg shadow p-4 sm:p-8 bg-[var(--color-card)]">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-2">
        <h1 className="text-2xl font-bold text-[var(--color-card-foreground)]">Sales Orders</h1>
        <a
            ...
            {isEmpty ? (
              <tr>
                <td colSpan={5} className="py-12 text-center text-[var(--color-muted-foreground)] text-lg">
                  ...
                  <a href="/dashboard/sales-orders/create" className="text-[var(--color-primary)] underline">Create your first sales order</a>
                </td>
              </tr>
            ) : (
              allSalesOrders.map((order, idx) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap font-mono text-[var(--color-card-foreground)]">{order.orderNumber}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-[var(--color-card-foreground)]">
                    {typeof order.items?.reduce === 'function'
                      ? order.items.reduce((sum, item) => sum + (Number(item.amount) || 0), 0).toLocaleString("en-US", { style: "currency", currency: "USD" })
                      : "$0.00"}
                  </td>
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-44 p-2" align="end">
                        <div className="flex flex-col gap-1">
                          <a href="#" className="px-3 py-2 rounded hover:bg-[var(--color-muted)] text-[var(--color-primary)] text-sm" aria-label="Preview Sales Order">Preview</a>
                          <Link href={`/dashboard/sales-orders/edit/${order.orderNumber}`} className="px-3 py-2 rounded hover:bg-[var(--color-muted)] text-[var(--color-muted-foreground)] text-sm" aria-label="Edit Sales Order">Edit</Link>
